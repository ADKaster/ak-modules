...
export constexpr auto is_any_of(StringView values)
{
    return [values](auto c) { return values.contains(c); };
}

export constexpr auto is_not_any_of(StringView values)
{
    return [values](auto c) { return !values.contains(c); };
}

export constexpr auto is_path_separator = is_any_of("/\\"sv);
export constexpr auto is_quote = is_any_of("'\""sv);

}

#if USING_AK_GLOBALLY
export using AK::GenericLexer;
export using AK::is_any_of;
export using AK::is_path_separator;
export using AK::is_quote;
#endif
